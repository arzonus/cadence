...
func TestDeleteExecutors(t *testing.T) {
	...
	t.Run("DeletesMultipleExecutors", func(t *testing.T) {
		...
		require.NoError(t, executorStore.AssignShard(ctx, tc.Namespace, store.AssignShardRequest{ShardID: shardOfDeletedExecutor1, ExecutorID: execToDelete1}))
		require.NoError(t, executorStore.AssignShard(ctx, tc.Namespace, store.AssignShardRequest{ShardID: shardOfDeletedExecutor2, ExecutorID: execToDelete2}))
		require.NoError(t, executorStore.AssignShard(ctx, tc.Namespace, store.AssignShardRequest{ShardID: shardOfSurvivingExecutor, ExecutorID: execToKeep}))
...
func TestAssignAndGetShardOwnerRoundtrip(t *testing.T) {
	...
	err = executorStore.AssignShard(ctx, tc.Namespace, store.AssignShardRequest{ShardID: shardID, ExecutorID: executorID})
...
func TestAssignShardErrors(t *testing.T) {
	...
	require.NoError(t, err)
	err = executorStore.AssignShard(ctx, tc.Namespace, store.AssignShardRequest{ShardID: shardID1, ExecutorID: activeExecutorID})
	require.NoError(t, err)
	...
	err = executorStore.AssignShard(ctx, tc.Namespace, store.AssignShardRequest{ShardID: shardID2, ExecutorID: "non-existent-executor"})
	...
	err = executorStore.AssignShard(ctx, tc.Namespace, store.AssignShardRequest{ShardID: shardID2, ExecutorID: drainingExecutorID})
...
func TestShardStatisticsPersistence(t *testing.T) {
	...
	stats := store.ShardStatistics{SmoothedLoad: 12.5, UpdatedTime: time.Unix(1234, 0).UTC(), LastAssignmentTime: time.Unix(5678, 0).UTC()}
	...
	require.NoError(t, executorStore.AssignShard(ctx, tc.Namespace, store.AssignShardRequest{ShardID: shardID, ExecutorID: executorID}))
	...
	assert.Equal(t, stats.UpdatedTime, updatedStats.UpdatedTime)
	// This should be greater than the last move time
	assert.Greater(t, updatedStats.LastAssignmentTime, stats.LastAssignmentTime)
